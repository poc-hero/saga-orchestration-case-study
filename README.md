...
| **site-service** | Service site (`com.pocmaster.site`) : Saga en 3 étapes — validation+création, ACL, indexation (composante interne). Port 9090. |
| **uaa-service** | Service UAA (`com.pocmaster.uaa`) : règles d'accès / ACL (port 9091). |

**Étapes de la Saga :** 1) validation + création (site), 2) ACL (UAA), 3) indexation (dans site-service ; paramètre **failIndexation**=true pour forcer l’échec et illustrer les compensations). En cas d'échec, compensations en ordre inverse (indexation → ACL → validation+création).

Pour activer les composants Saga : **@EnableSagaExecutor** sur la classe principale (ex. `SiteApplication`).
...
4. Appel de la Saga (succès) :  
   `curl -X POST http://localhost:9090/api/site/create -H "Content-Type: application/json" -d '{"companyId":"c1","siteName":"Mon site"}'`
5. Pour illustrer les compensations (échec volontaire à l’indexation) :  
   `curl -X POST http://localhost:9090/api/site/create -H "Content-Type: application/json" -d '{"companyId":"c1","siteName":"Mon site","failIndexation":true}'`
